...
export default function ChemistNavbar({ onMenuClick, sidebarOpen }) {
  ...
  // Chemist-specific notifications
  const notifications = [
    {
      id: 1,
      message: "New medicine order received",
      time: "10 min ago",
      read: false,
      type: "order"
    },
    {
      id: 2,
      message: "Stock alert: Paracetamol running low",
      time: "2 hours ago",
      read: false,
      type: "stock"
    },
    ...
    {
      id: 5,
      message: "New supplier order confirmed",
      time: "2 days ago",
      read: true,
      type: "supplier"
    },
  ...
  const getNotificationIcon = (type) => {
    switch(type) {
      ...
      case 'stock': return <AlertCircle className="w-4 h-4 text-amber-600" />;
      case 'prescription': return <Pill className="w-4 h-4 text-green-600" />;
      case 'report': return <Beaker className="w-4 h-4 text-purple-600" />;
