...
export async function analyzeHealthData(assessmentType, inputs, healthScore, riskFactors) {
  try {
    const prompt = `
You are a ${assessmentType === 'heart' ? 'cardiology' : 'pulmonology'} health expert. Analyze the health data and provide comprehensive medical analysis.
...
- Medical History: ${inputs.hypertension_history ? 'Hypertension, ' : ''}${inputs.diabetes_history ? 'Diabetes, ' : ''}${inputs.family_cardiac_history ? 'Family History' : 'None'}
- Symptoms: ${inputs.chest_pain ? 'Chest pain, ' : ''}${inputs.breathlessness ? 'Breathlessness, ' : ''}${inputs.palpitations ? 'Palpitations' : 'None'}
` : `
- Age: ${inputs.age}
- Gender: ${inputs.gender}
- Height: ${inputs.height_cm} cm, Weight: ${inputs.weight_kg} kg
- Smoking: ${inputs.smoking_status} (${inputs.smoking_pack_years || 0} pack-years)
...
- Symptoms: ${inputs.cough_frequency} cough, ${inputs.breathlessness} breathlessness, ${inputs.wheezing ? 'Wheezing' : 'No wheezing'}
...
export async function generateHealthRecommendations(assessmentType, inputs, riskFactors) {
  try {
    const prompt = `
...
${assessmentType === 'heart' ? `
- Blood Pressure: ${inputs.systolic_bp}/${inputs.diastolic_bp} mmHg
- Lifestyle: ${inputs.smoking_status} smoking, ${inputs.physical_activity_minutes} mins/week activity
- Risk Factors: ${riskFactors.join(', ')}
...
- Breathing: ${inputs.breath_holding_time}s hold, ${inputs.breaths_per_minute} bpm
- Risk Factors: ${riskFactors.join(', ')}
- Indian Context: Provide culturally appropriate advice for Indian users
`}
...
function generateFallbackAnalysis(assessmentType, healthScore, riskFactors) {
  return {
    analysis: `Your ${assessmentType} health score is ${healthScore}/100. ${
      healthScore >= 70 ? 'Excellent health! Maintain your current lifestyle.' :
      healthScore >= 50 ? 'Good health with some areas for improvement.' :
    ...
    medical_attention: "Consult a doctor if you experience persistent symptoms."
  };
}
...
function generateFallbackRecommendations(assessmentType, riskFactors) {
  ...
  if (assessmentType === 'heart') {
    if (riskFactors.includes('High blood pressure')) {
      recommendations.push({
        category: "lifestyle",
        title: "Reduce Sodium Intake",
        description: "Lower salt consumption to help control blood pressure",
        priority: "high",
        action_steps: ["Limit processed foods", "Use herbs instead of salt", "Choose fresh vegetables"],
        timeframe: "immediate",
        indian_context: true
      });
    }
    ...
    if (riskFactors.includes('Smoking')) {
      recommendations.push({
        ...
        priority: "high",
        action_steps: ["Seek cessation programs", "Avoid smoking triggers", "Practice breathing exercises"],
        timeframe: "1-week",
        indian_context: true
      });
    }
